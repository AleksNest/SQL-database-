...
FROM cats c
INNER JOIN shops s ON s.id = c.shops_id;

-- ЗАДАНИЕ 2   Вывести магазин, в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
SELECT  s.shopname, c.name				-- 1 способ
FROM cats c
LEFT JOIN shops s ON s.id = c.shops_id
WHERE name = "Murzik";

SELECT s.shopname, c.name 				-- 2 способ
FROM shops s
INNER JOIN (										 
			SELECT name, shops_id
            FROM cats
            WHERE  "Murzik" IN (name)
) c
ON c.shops_id = s.id;


-- ЗАДАНИЕ 3   Вывести магазины, в котором НЕ продаются коты “Мурзик” и “Zuza”
SELECT  s.shopname, c.name				-- 1 способ
FROM cats c
LEFT JOIN shops s ON s.id = c.shops_id
WHERE shops_id != (SELECT shops_id FROM cats WHERE name ="Murzik" ) AND  shops_id != (SELECT shops_id FROM cats WHERE name ="Zuza" );
...
INNER JOIN (										 
			SELECT name, shops_id
            FROM cats
            WHERE shops_id != (SELECT shops_id FROM cats WHERE "Murzik" IN (name) ) AND  shops_id != (SELECT shops_id FROM cats WHERE "Zuza" IN (name) )
) c
ON c.shops_id = s.id;

-- ЗАДАНИЕ 4 Вывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.
-- 1 вариант вывод списка анализов без повтора
SELECT  DISTINCT a.an_name, a.an_cost, a.an_price 			
FROM analysis a
INNER JOIN orders o ON a.an_id = o.ord_an
WHERE o.ord_datetime BETWEEN '2020-02-05 00:00:00' AND  '2020-02-12 23:59:59';

-- 2 вариант вывод списка анализов c расчетом количества каждого из анализа в указанные даты
SELECT  a2.an_name, a2.an_cost, a2.an_price, count(*)
FROM (SELECT a1.an_name, a1.an_cost, a1.an_price 			
FROM analysis a1
INNER JOIN orders o ON a1.an_id = o.ord_an
WHERE o.ord_datetime BETWEEN '2020-02-05 00:00:00' AND  '2020-02-12 23:59:59') a2
GROUP BY a2.an_name, a2.an_cost, a2.an_price
HAVING count(*) >= 1;
